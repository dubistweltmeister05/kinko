...
Hello there lads! My name is Kshitij Neeraj Vaze, and I used to work as a Linux Kernel developer at a hardware startup, based in Surat, Gujarat. It’s been just 2 weeks since I started my Job, and one of the first things that I encountered was the process of _board bring-up,_ which is a collection of techniques that us engineers use to take a brand-new development board, and work with it to get it to boot (Kinda, sort of, not really tbh, there’s much more to this).
...
Following are a set of notes that I made all throughout the week (in a more structured manner), as I attempt to present to you, my reader, the complete working of our beloved Linux Kernel Stack (idk if y’all love it, but I certainly do now!)

It is a collection of notes from 2 articles from the amazing lwn.net, which I have linked at the end. I hope you stick around till then, and check out the blog.
...
[](https://medium.com/plans?source=promotion_paragraph---post_body_banner_unlock_stories_scribble--5e9360ad0d94---------------------------------------)
...
The entire stack — from shader programs to DMA memory, from buffer objects to screen tiles — is a glorious ballet of memory management, clever abstractions, and low-level hardware control. Linux doesn’t just draw pixels — it orchestrates a pixel symphony. A symphony that gets most of us software engineers paid!
